...
class Model {
    ...
    public function flush() {
        ...
        if(C('DB_FIELDS_CACHE')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?:C('DB_NAME');
            F('_fields/'.strtolower($this->getTableName()),$this->fields);
        }
    }
    ...
    public function table($table) {
        ...
        if(is_array($table)) {
            ...
            if(C('DB_TYPE')=='dm')
            {
                $table  = preg_replace_callback("/__([A-Z0-9_-]+)__/sU", function($match) use($prefix){ return $prefix.$match[1];}, $table);
            }else{
    ...
    public function join($join,$type='INNER') {
        ...
        if(is_array($join)) {
            ...
            if(C('DB_TYPE')=='mysql')
            {
                $join  = preg_replace_callback("/__([a-zA-Z0-9_-]+)__/sU", function($match) use($prefix,$dbname){ return $dbname.'.`'.$prefix.strtolower($match[1]).'`';}, $join);
            }else{
                $join  = preg_replace_callback("/__([a-zA-Z0-9_-]+)__/sU", function($match) use($prefix,$dbname){ return '"'.$dbname.'"."'.$prefix.$match[1].'"';}, $join);
            }
