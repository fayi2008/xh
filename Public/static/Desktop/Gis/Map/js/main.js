$(function(){
    ...
    if(ua.shell = 'ie' && ua.version<9){
        var hotScroll = {
            'refresh':function(){}
        }
        $('#j_hot').css({'overflow-y':'auto'});
    }else{
        var hotScroll = new IScroll('#j_hot',{
            click:true,
            scrollX:false,
            scrollY:true,
    ...
    function render(){
        ui.render({
            'view':$('#tpl_hot_span').html(),
            'wrap':'#j_hot .wrap',
            'model':'/Gis/poi/getTag',
            'callback':function(data){
                labelManager.loadSpan(data);
                labelManager.load('0');
                hotScroll.refresh();
