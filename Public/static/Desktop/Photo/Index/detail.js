...
$(function() {
    function Detail() {
        ...
        function view() {
            ...
            $(".data-author-name").html("作者：" + datas.nickname + '<span class="times">' + $.formatDate(new Date(datas.create_time * 1000), "yyyy-MM-dd  hh:mm:ss") + "</span>");
            var html = '<div class="img"><img src="' + $._LOCAL_IMGURL_(datas.file_url) + '"></div>';
            html += '<div class="desc">' + datas.description + "</div>";
            $(".data-author-content").html(html);
                    ...
                    if (rs.data.data) {
                        if (rs.data.data.length) {
                            ...
                            for (var i in rs.data.data) {
                                ...
                                html += '<div class="comment-item-user"><span class="nickname">' + datas.nickname + '</span><span class="create_time">发表时间：' + $.formatDate(new Date(datas.create_time * 1e3), "yyyy-MM-dd HH:mm:ss") + '</span><span class=""></span>';
                                html += "</div>";
